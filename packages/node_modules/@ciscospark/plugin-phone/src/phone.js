...
const Phone = SparkPlugin.extend({
  ...
  share(device, options = {}) {
    ...
    if (typeof device === 'object') {
      promiseOrId = device.id;
    }
    else if (patterns.uuid.test(device)) {
      promiseOrId = device;
    }
    else if (this.spark.internal.search) {
      promiseOrId = this.spark.internal.search.people({
        includePeople: false,
        includeRooms: true,
        searchEntity: 'device',
        query: device
        ...
        .then((devices) => {
          const lyraSpaces = devices.filter((d) => d.machineType === 'LYRA_SPACE');
          switch (lyraSpaces.length) {
            case 0:
              return Promise.reject(new Error(`No devices could be found matching "${device}"`));
            case 1:
              return lyraSpaces[0].id;
            default:
              return Promise.reject(new Error(`Found more than one device matching "${device}"`));
          }
        });
    }
    else {
      return Promise.reject(new Error(`"${device}" does not appear to identify a device`));
    }

    const call = Call.make({}, {parent: this.spark});

    Promise.resolve(promiseOrId)
      .then((id) => call.share(id, options))
      .catch((err) => {
        call.trigger('error', err);
      });
