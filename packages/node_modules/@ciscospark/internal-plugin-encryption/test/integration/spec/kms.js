...
describe(`Encryption`, function() {
  ...
  describe(`KMS`, () => {
    ...
    describe(`#addAuthorization()`, () => {
      ...
      it(`authorizes a user to a key`, () => spark.internal.encryption.kms.addAuthorization({
        ...
        .then(([auth]) => {
          ...
          return assert.isFulfilled(mccoy.spark.internal.encryption.kms.fetchKey({uri: boundedKeyUri}));
        }));
    ...
    describe(`#listAuthorizations()`, () => {
      ...
      describe.only(`when used in parallel`, () => {
        let kro1, kro2, kro3, kro4, kro5;
        before(`prepare kros`, () => spark.internal.encryption.kms.createUnboundKeys({count: 5})
          .then((keys) => Promise.all([
            spark.internal.encryption.kms.createResource({
              userIds: [spark.internal.device.userId],
              key: keys[0]
            }),
            spark.internal.encryption.kms.createResource({
          ...
          .then((kros) => {
            [kro1, kro2, kro3, kro4, kro5] = kros;
          }));

        it(`returns when the current user has access to all resources`, () => Promise.all([
          spark.internal.encryption.kms.listAuthorizations({kroUri: kro1.uri}),
          spark.internal.encryption.kms.listAuthorizations({kroUri: kro2.uri}),
          spark.internal.encryption.kms.listAuthorizations({kroUri: kro3.uri}),
          spark.internal.encryption.kms.listAuthorizations({kroUri: kro4.uri})
        ]));

        it(`rejects when the current user has access to none of the resources`, () => Promise.all([
          assert.isRejected(mccoy.spark.internal.encryption.kms.listAuthorizations({kroUri: kro1.uri})),
          assert.isRejected(mccoy.spark.internal.encryption.kms.listAuthorizations({kroUri: kro2.uri})),
          assert.isRejected(mccoy.spark.internal.encryption.kms.listAuthorizations({kroUri: kro3.uri})),
          assert.isRejected(mccoy.spark.internal.encryption.kms.listAuthorizations({kroUri: kro4.uri}))
        ]));

        it(`rejects when the current user has access to some of the resources`, () => Promise.all([
          assert.isRejected(mccoy.spark.internal.encryption.kms.listAuthorizations({kroUri: kro1.uri})),
          assert.isRejected(mccoy.spark.internal.encryption.kms.listAuthorizations({kroUri: kro2.uri})),
          assert.isRejected(mccoy.spark.internal.encryption.kms.listAuthorizations({kroUri: kro3.uri})),
          assert.isRejected(mccoy.spark.internal.encryption.kms.listAuthorizations({kroUri: kro4.uri})),
          mccoy.spark.internal.encryption.kms.listAuthorizations({kroUri: kro5.uri})
        ]));
      });
    });
    ...
    describe(`#ping()`, () => {
      it(`sends a ping to the kms`, () => assert.isFulfilled(spark.internal.encryption.kms.ping())
        .then((res) => {
    ...
    describe(`when ecdhe negotiation times out`, () => {
      ...
      it(`handles late ecdhe responses`, () => assert.isFulfilled(spark2.internal.encryption.kms.ping())
        .then(() => {
    ...
    describe(`when the kms is in another org`, () => {
      ...
      it(`responds to pings`, () => assert.isFulfilled(fedSpark.internal.encryption.kms.ping())
        .then((res) => {
          assert.property(res, `status`);
          assert.equal(res.status, 200);
          assert.property(res, `requestId`);
