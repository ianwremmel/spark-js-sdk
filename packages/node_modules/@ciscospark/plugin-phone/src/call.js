...
const Call = SparkPlugin.extend({
  ...
  createOrJoinLocus(target, options = {}) {
    if (options.localMediaStream) {
      ...
      if (options.constraints.mediaSource === 'screen' || options.constraints.mediaSource === 'application') {
   ...
   */
  share(id, options = {}) {
    this.createOrJoinLocus(id, {
      callPreferences: {
        requestedMedia: ['SLIDES']
      },
      constraints: {
        audio: false,
        video: defaults({}, options.constraint, {
      ...
      .then(() => new Promise((resolve) => {
        // FIXME merge this with waitForMediaShare. Will need to
        // programmatically determine if we intend to use a separate device
        const findShare = () => {
          try {
            const deviceParticipant = this.locus.participants.find((p) => p.identity === id);
            const selfDevice = deviceParticipant && this.locus.self.devices.find((d) => d.intent.associatedWith === deviceParticipant.url);

            if (!selfDevice) {
              this.once('change:locus', findShare);
              return;
            }

            if (selfDevice.state !== 'JOINED') {
              this.once('change:locus', findShare);
              return;
            }

            if (selfDevice.intent.type !== 'OBSERVE') {
              this.once('change:locus', findShare);
              return;
            }
            const mediaShare = this.locus.mediaShares.find((share) => share.name === 'content');
            if (!mediaShare) {
              this.once('change:locus', findShare);
              return;
            }
